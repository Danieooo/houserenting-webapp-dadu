...
- [ ] Khởi tạo thư viện kiểm thử Playwright trong dự án.
- [ ] Viết các kịch bản kiểm thử tích hợp tự động cho luồng Happy Path cốt lõi:
...
- [ ] Thiết lập chạy test tự động Playwright trên môi trường local trước mỗi lượt commit.
- [ ] _Requirements: [R6.2, R1.2, R2.2, R3.3, R3.4]_
...
- [ ] Viết kịch bản GitHub Actions tự động gọi API sao lưu hàng ngày (`daily-backup.yml`) lúc 2:00 AM, xuất dữ liệu database Neon PostgreSQL thành tệp SQL/CSV và tự động tải lên Google Drive của chủ trọ thông qua Google API Service Account.
- [ ] Nghiên cứu tích hợp nút bấm "Gửi thông báo hóa đơn" qua webhook Zalo ZNS hoặc SMS (sử dụng các API gửi tin nhắn miễn phí hoặc trả phí rẻ) để chủ trọ chỉ cần click là khách thuê nhận được tin nhắn báo tiền nhà kèm link xem hóa đơn chi tiết.
- [ ] _Requirements: [R3.6, R4.2]_
