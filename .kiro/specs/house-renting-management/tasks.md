...
### Task 17: Linh hoạt hóa thông tin liên hệ khách thuê & cải thiện luồng Zalo (Tenant Contact Flexibility & Zalo UX)
- [ ] Cập nhật đặc tả dữ liệu khách thuê để `phone` trở thành trường không bắt buộc và bổ sung trường `zaloContact` tách biệt.
- [ ] Điều chỉnh backend/frontend validation để cho phép tạo khách thuê không có số điện thoại nhưng vẫn lưu được thông tin liên hệ Zalo dạng text tự do.
- [ ] Nâng cấp modal gửi thông báo hóa đơn để luôn hỗ trợ sao chép nội dung, chỉ mở chat Zalo/SMS khi có `phone`, và hiển thị `zaloContact` làm thông tin định vị cuộc trò chuyện khi không có số điện thoại.
- [ ] Bổ sung test UI/backend cho các trường hợp: có `phone`, chỉ có `zaloContact`, và không có cả hai.
- [ ] _Requirements: [R2.2, R7.2, R7.3]_
